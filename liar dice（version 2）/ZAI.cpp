...
void ZAI::go_test_only()
{
	//先计算概率
	double prob;
	int chazhi = ui->lassguess.count - dice[ui->lassguess.value] - ui->lassguess.value == 0 ? 0 : dice[0];
	prob = 1 - normal_cdf((chazhi - 5.0 / 3) / (10.0 / 9));
	printf("prob = %lf\n", prob);
	...
	for (const auto& x : MaybeGuess)
	{
		...
		s = 1.0 - normal_cdf((chazhi - 5.0 / 3.0) / (10.0 / 9));
		printf("chazi = %lf\n",chazhi);
